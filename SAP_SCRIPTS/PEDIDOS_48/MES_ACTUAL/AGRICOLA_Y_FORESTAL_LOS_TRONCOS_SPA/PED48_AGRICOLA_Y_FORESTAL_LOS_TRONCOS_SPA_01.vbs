...
' ============ PERIODO DEL PEDIDO ============
' d1 = primer dia del mes; d2 = ultimo dia del mes (fecha de entrega)
hoy = Date
...
contrato = Trim(InputBox("Pegue el numero del contrato marco (46xxxxxxxx) del proveedor:" & vbCrLf & vbCrLf & "AGRICOLA Y FORESTAL LOS TRONCOS SPA", "Contrato marco para el pedido 48"))
...
session.findById("wnd[0]").sendVKey 0
Call ClosePopups()
MsgBox "Pedido con referencia al contrato " & contrato & " cargado con " & (UBound(items)+1) & " posiciones (contrato+posicion+cantidad; material, precio y centro los trae SAP desde el contrato)." & vbCrLf & "Periodo del pedido: " & d1 & " al " & d2 & ". Fecha de entrega escrita en cada posicion: " & d2 & "." & vbCrLf & "Revise las posiciones y los mensajes de SAP. Si esta correcto, presione GUARDAR manualmente.", 64, "Revision manual"
...
Sub FillItems(arr)
Dim tbl, i, r, firstRow, vis, colKonnr, colKtpnr, colMenge, colFecha
...
If tbl Is Nothing Then
    ...
    WScript.Quit 1
End If
Call MapColumns(tbl, colKonnr, colKtpnr, colMenge, colFecha)
...
For i = 0 To UBound(arr)
    ...
    If colFecha >= 0 Then SetCell tbl, r, colFecha, d2
Next
End Sub
...
Sub MapColumns(tbl, colKonnr, colKtpnr, colMenge, colFecha)
On Error Resume Next
Dim c, nm, total
colKonnr = -1
colKtpnr = -1
...
For c = 0 To total - 1
    ...
    If Err.Number = 0 And nm <> "" Then
        ...
        If colFecha < 0 And (InStr(nm, "EEIND") > 0 Or InStr(nm, "EINDT") > 0) Then colFecha = c
    End If
Next
On Error GoTo 0
End Sub
