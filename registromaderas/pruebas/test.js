...
/**
 * apiLote con el tipo puesto: ahora la tanda se elige antes de pegar. Por
 * defecto PT, que es lo mas comun; los de proceso pasan 'PP'.
 */
function lote_(entrada, tipo) {
  const datos = (typeof entrada === 'string') ? { codigos: entrada } : Object.assign({}, entrada);
  if (!datos.tipo) datos.tipo = tipo || 'PT';
  return apiLote(datos);
}
...
seccion('Contexto que recibe el formulario');
{
  ...
  ok(Object.keys(ctx.monitor).join() === 'url',
    'y nada más: el aviso espera el clic, no un reloj');
...
seccion('Los ceros se ponen solos');
{
  ...
  const lote = lote_({ codigos: 'RSJR32X180X3200',
...
}
{
  ...
  const r = lote_('RSFR037X130X3200\tRVF 037X130\tRSF 037X130\t248');
...
}
{
  ...
  const r = lote_('C4JR019X100X2440');
...
}
{
  ...
  const r = lote_('RVMH032X180X3960');
...
seccion('Qué columnas de ruta pide el lote');
{
  ...
  const rustico = lote_({ codigos: 'RSJR032X180X3200' });
  ...
  const mezcla = lote_({ codigos: 'RVMH032X180X3960\nRSJR032X180X3200' });
...
// La tanda es de un solo tipo, elegido ANTES de pegar: mezclar terminados con
// material de proceso pediria rutas distintas por linea.
seccion('El tipo se elige antes, y manda sobre toda la tanda');
{
  ok(error(() => apiLote({ codigos: 'RVMH032X180X3960' })).indexOf('Elige primero') !== -1,
    'sin elegir tipo no se puede analizar');
  ok(error(() => apiLote({ codigos: 'RVMH032X180X3960', tipo: 'XX' })).indexOf('Elige primero') !== -1,
    'ni con un tipo inventado');
...
{
  const pt = lote_('RSJR032X180X3200');
  ...
  ok(especial.filas[0].umb === 'M3' && !especial.filas[0].pidePak,
    'y el especial también va en m3, sin PAK');
}

seccion('Revisar el lote después de completar las rutas');
{
  const lote = lote_('RSJR032X180X3200');
...
seccion('El Excel sale de las filas seleccionadas en la hoja');
{
  ...
  // El xlsx lo genera Google, no este codigo: se copian las filas a una hoja
  // temporal y se pide su exportacion a Excel.
  const exportado = () => JSON.parse(
    exportarComoExcel_(filasSeleccionadas_().filas, 'prueba').getDataAsString());

  SS.__seleccionar('PT', [[a.fila, 1], [c.fila, 1]]);
  const x = exportado();
  ok(x.hoja === 'Batch input', 'la hoja temporal se llama como el batch input');
  ok(x.datos.length === 4 && !x.datos[0].length && !x.datos[1].length,
    'la 1 y la 2 quedan en blanco');
  ok(x.datos[2][0] === 'CL' && x.datos[3][0] === 'CL', 'y los datos empiezan en la fila 3');
  ok(x.datos[2][22] === '3100' && x.datos[3][22] === '3500', 'en el orden de la hoja');
  ok(x.formatos['3,1'] === '@' && x.formatos['3,9'] === '@',
    'todo entra como texto, que es lo que salva los ceros a la izquierda');
  ok(x.datos[2][8] === '037', 'y en efecto el 037 llega con su cero');

  ok(SpreadsheetApp.__temporales().every(t => t.enPapelera),
    'la hoja temporal se borra despues de exportar');

  const antes = SpreadsheetApp.__temporales().length;
  global.__FALLA_EXPORTACION = 500;
  ok(error(() => exportado()).indexOf('Google respondio 500') !== -1,
    'si Google no exporta, se avisa en vez de entregar un archivo roto');
  delete global.__FALLA_EXPORTACION;
  ok(SpreadsheetApp.__temporales().length === antes + 1 &&
     SpreadsheetApp.__temporales().every(t => t.enPapelera),
    'y aun asi la temporal no queda tirada en el Drive');
}
